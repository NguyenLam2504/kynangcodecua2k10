...
local Tabs = {
    ...
    Stats = Window:AddTab({ Title = "Sub Farming ⚔️", Icon = "" }),
    Seaev = Window:AddTab({ Title = "Sea Event ⚓", Icon = "" }),
    Player = Window:AddTab({ Title = "Player 👨", Icon = "" }),
    Teleport = Window:AddTab({ Title = "Travel ✈️", Icon = "" }),
    ...
    Setting = Window:AddTab({ Title = "Settings ⚙️", Icon = "" }),
...
Fluent:Notify({
    Title = "Niga Bot Notify",
    Content = "Thank mbm hub for fluent lib 💞",
    Duration = 5
})
...
local listfastattack = {'0.25','0.2','0.15','0.1','0.05','0.02','0.015','0.01','0'}
    ...
    spawn(function()
        ...
        --------------------------------------------------------------------------------------------------------------------------------------------
        local ToggleMobAura = Tabs.Main:AddToggle("ToggleMobAura", {Title = "Farm Mob Aura", Description = "farm quái xung quanh đảo", Default = false })
...
if First_Sea then
    local Togglesea2go = Tabs.Stats:AddToggle("Togglesea2go", {Title = "Auto Quest Sea 2", Default = false })

    TogglTogglesea2go:OnChanged(function(Value)
        _G.AutoSecondSea = Value
    end)
    Options.Togglesea2go:SetValue(false)
        spawn(function()
            while wait() do 
...
if Second_Sea then
    local Togglesea3go = Tabs.Main:AddToggle("Togglesea3go", {Title = "Auto Quest Sea 3", Default = false })

    Toggle:OnChanged(function(Value)
        _G.AutoThirdSea = Value
    end)
    Options.Togglesea3go:SetValue(false)
        spawn(function()
            while wait() do
...
spawn(function()
    ...
    local ToggleMaterial = Tabs.Raid:AddToggle("ToggleMaterial", {Title = "Auto Material", Description = "cày vật liệu đã chọn", Default = false })
    ...
    spawn(function()
      ...
      local EliteProgress = Tabs.Stats:AddParagraph({
    ...
    spawn(function()
      ...
      local Elite_Hunter_Status = Tabs.Stats:AddParagraph({
        Title = "Elite Hunter",
        Content = ""
    })
    spawn(function()
    ...
    local ToggleElite = Tabs.Stats:AddToggle("ToggleElite", {Title = "Auto Elite Hunter", Description = "cày elite", Default = false })
  ...
  Tabs.Seaev:AddButton({
    Title = "Trade Azure",
    Description = "đổi lửa xanh",
    Callback = function()            
game:GetService("ReplicatedStorage"):WaitForChild("Modules"):WaitForChild("Net"):WaitForChild("RF/KitsuneStatuePray"):InvokeServer()
	...
	spawn(function()
        ...
        local ToggleTushita = Tabs.Stats:AddToggle("ToggleTushita", {Title = "Auto Tushita (Kill Boss Longma)", Description = "lấy kiếm tushita", Default = false })
        ...
        Options.ToggleTushita:SetValue(false)
           spawn(function()
                ...
                local ToggleAuto_Cursed_Dual_Katana = Tabs.Stats:AddToggle("ToggleAuto_Cursed_Dual_Katana", {Title = "Auto CDK (Beta) ",Description = "Lấy song kiếm nguyền rủa", Default = false })
                ToggleAuto_Cursed_Dual_Katana:OnChanged(function(Value)
                    Auto_Cursed_Dual_Katana = Value
        end)
        Options.ToggleAuto_Cursed_Dual_Katana:SetValue(false)
        spawn(function()
            while wait() do
                pcall(function()
...
local ToggleAutotrial = Tabs.Race:AddToggle("ToggleAutotrial", {Title = "Auto Complete Trial", Description = "hoàn thành trial", Default = false })
...
local ToggleAutotrialafterkill = Tabs.Race:AddToggle("ToggleAutotrialafterkill", {Title = "Kill Player After Complete Trial", Description = "đánh người chơi hoàn thành trial", Default = false })
...
elseif _G.shopping == "Black Cape" then
	...
	elseif _G.shopping == "Super Human" then
        wait(0.5)
    ...
    elseif _G.shopping == "God Human" then
        wait(0.5)
...
local Inputjobid = Tabs.Hop:AddInput("Input", {
    Title = "job id",
    Default = "",
...
if Third_Sea then
    local RoughSea = Tabs.Seaev:AddSection("Rough Sea")

    local ToggleSailBoat = Tabs.Seaev:AddToggle("ToggleSailBoat", {Title = "Auto Buy Boat + Travel Zone 6",Description = "Mua thuyền và tele đến Zone 6", Default = false })
...
game:GetService("RunService").RenderStepped:Connect(function()
    ...
    local ToggleAutoTerrorshark = Tabs.Seaev:AddToggle("ToggleAutoTerrorshark", {Title = "Kill Terror shark",Description = "Đánh Terror shark", Default = false })

    ToggleAutoTerrorshark:OnChanged(function(Value)
        _G.AutoTerrorshark = Value
    end)
    Options.ToggleAutoTerrorshark:SetValue(false)
    spawn(function()
        while wait() do
    ...
    local TogglePiranha = Tabs.Seaev:AddToggle("TogglePiranha", {Title = "Auto Kill Piranha",Description = "đánh cá Piranha", Default = false })
     ...
     local ToggleSeaBeAst = Tabs.Seaev:AddToggle("ToggleSeaBeAst", {Title = "Auto Kill Sea Beast + Ghost Ship",Description = "Kill sea beast + thuyền ma", Default = false })
     ...
     ToggleSeaBeAst:OnChanged(function(Value)
         ...
         local ToggleFishCrew = Tabs.Seaev:AddToggle("ToggleFishCrew", {Title = "Auto Kill Fish Crew",Description = "đánh cá crew", Default = false })
         ...
         local ToggleShip = Tabs.Seaev:AddToggle("ToggleShip", {Title = "Auto Kill Ship",Description = "đánh thuyền", Default = false })
...
Tabs.Misc:AddButton({
    ...
    Description = "Join my server discord if u want / vào sv discord nếu bạn muốn",
    Callback = function()
        setclipboard("https://discord.gg/9PzEWrzgXR")
    end
  })
